# Forked from https://github.com/Julian-Wyatt/AnoDDPM/blob/3052f0441a472af55d6e8b1028f5d3156f3d6ed3/simplex.py
...
class Simplex_CLASS:
    ...
    def rand_3d_octaves(self, shape, octaves=1, persistence=0.5, frequency=32):
        ...
        noise = np.zeros(shape, dtype=np.float32)       # (3, 1, 128, 128)
        z, y, x = [np.arange(0, shape[idx]) for idx in [0, 2, 3]]
        amplitude = 1
        for _ in range(octaves):
            noise[:, 0] += amplitude * self.noise3array(x / frequency, y / frequency, z / frequency)
            frequency /= 2
    ...
    def rand_3d_fixed_T_octaves(self, shape, T, octaves=1, persistence=0.5, frequency=32):
        ...
        """
        noise = np.zeros(shape)
        y, x = [np.arange(0, end) for end in shape[-2:]]
        amplitude = 1
        for _ in range(octaves):
